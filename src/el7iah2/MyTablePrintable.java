...
class MyTablePrintable implements Printable {
    ...
    public int print(Graphics graphics, PageFormat pageFormat, int pageIndex)
        ...
        if (headerFormat != null) {
            ...
            for(int i=0 ; i<headerFormat.length ; i++ )
                headerText[i] = headerFormat[i][0].format(pageNumber);
        }
        ...
        if (footerFormat != null) {
            ...
            for(int i=0 ; i<footerFormat.length ; i++ )
                footerText[i] = footerFormat[i][0].format(pageNumber);
        }
        ...
        g2d.setFont(table.getFont().deriveFont(Font.BOLD, 14f));
        ...
        if (footerText != null) {
            ...
            for(int i=0 ; i<footerText.length ; i++)
                printText(g2d, footerText[i], fRect[i], footerFont, Integer.parseInt(footerFormat[i][1].format(pageNumber))-1, imgWidth , Integer.parseInt(footerFormat[i][2].format(pageNumber)));
        ...
        if (headerText != null) {
            for(int i=0 ; i<headerText.length ; i++)
            {
                ...
                printText(g2d, headerText[i], hRect[i], headerFont, Integer.parseInt(headerFormat[i][1].format(pageNumber)), imgWidth,Integer.parseInt(headerFormat[i][2].format(pageNumber)));
    ...
    ,int position ) {
			...
			int tx = 10; // for left allign
if (position==1)tx = -(int)(Math.ceil(rect.getWidth()) - imgWidth)-10;
else if (position==2)tx =( -(int)(Math.ceil(rect.getWidth()) - imgWidth))/2;
